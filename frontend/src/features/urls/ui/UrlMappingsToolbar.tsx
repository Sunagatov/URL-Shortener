  ...
  return (
    <div className="mb-6 flex flex-col gap-2.5 sm:flex-row">
      <div className="relative flex-1">
        <FaSearch className="pointer-events-none absolute left-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[color:var(--text-muted)]" />
        <input
          type="text"
          value={search}
          onChange={event => onSearchChange(event.target.value)}
          placeholder="Search by original or short URL…"
          className="w-full rounded-xl border border-[color:var(--border)] bg-[var(--card-bg)] py-2.5 pl-10 pr-10 text-sm text-[color:var(--text-primary)] placeholder-[color:var(--text-muted)] transition-all focus:border-blue-500/25 focus:outline-none focus:ring-2 focus:ring-blue-500/30"
        />
        {search && (
          <button
            type="button"
      ...
      {displayTotal > 0 && (
        <div className="flex items-center rounded-xl border border-[color:var(--border)] bg-[var(--card-bg)] px-4 py-2.5">
          <span className="whitespace-nowrap text-xs text-[color:var(--text-muted)]">
            {isSearchMode
