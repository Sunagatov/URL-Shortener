...
export function LandingGuestCtaSection() {
  return (
    <section className="relative overflow-hidden bg-[var(--bg-alt)] py-16 bg-grid-dark md:py-28">
      ...
      <div className="absolute left-1/2 top-1/2 h-[400px] w-[700px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-600/12 blur-[130px] pointer-events-none dark:block hidden" />
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-6 text-center">
        ...
        <div className="flex flex-col justify-center gap-3 sm:flex-row">
          <Link to={routes.signUp}>
            <button className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-8 py-3.5 text-base font-semibold text-[color:var(--text-on-accent)] transition-all duration-200 hover:bg-blue-500 hover:shadow-[0_0_24px_rgba(59,130,246,0.35)]">
              Sign Up Free <FaArrowRight className="h-4 w-4" />
            </button>
          </Link>
          <Link to={routes.signIn}>
            <button className="inline-flex items-center gap-2 rounded-xl border border-[color:var(--border)] bg-[var(--surface-hover)] px-8 py-3.5 text-base font-semibold text-[color:var(--text-secondary)] transition-all duration-200 hover:bg-[var(--surface-hover)] hover:text-[color:var(--text-primary)]">
              Sign In
            </button>
          </Link>
        </div>
