  ...
  return (
    <div className="rounded-2xl border border-[color:var(--card-border)] bg-[var(--card-bg)]">
      <div className="border-b border-[color:var(--border)] px-5 py-3.5">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-[color:var(--text-muted)]">Top Performing Links</p>
      </div>
      <div className="p-5">
        {topLinks.items.length === 0 ? (
          ...
          <div className="space-y-3">
            {topLinks.items.map((item, i) => (
              <div key={item.urlHash} className="flex items-center gap-3 rounded-xl border border-[color:var(--border)] bg-[var(--card-bg)] p-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-medium text-[color:var(--text-secondary)]">{item.originalUrl}</p>
                  <p className="truncate text-[10px] text-[color:var(--text-muted)]" style={{ fontFamily: 'var(--font-mono)' }}>
                    /{item.urlHash}
                  </p>
                </div>
                <span className="flex-shrink-0 text-sm font-bold text-[color:var(--text-primary)]" style={{ fontFamily: 'var(--font-mono)' }}>
