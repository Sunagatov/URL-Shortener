...
class PasswordOfUserValidator : PasswordPolicyValidator {
    ...
    override fun validate(password: String) {
        when {
            password.isBlank() -> throw InvalidAuthRequestException(PASSWORD_MUST_NOT_BE_EMPTY)
            password.length < MIN_PASSWORD_LENGTH -> throw InvalidAuthRequestException(PASSWORD_MUST_BE_AT_LEAST_15_CHARACTERS_LONG)
            password.length > MAX_PASSWORD_LENGTH -> throw InvalidAuthRequestException(PASSWORD_IS_TOO_LONG)
        }
    }
}
